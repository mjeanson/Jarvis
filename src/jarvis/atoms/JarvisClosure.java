...
public class JarvisClosure extends JarvisAtom {

	private ArrayList<String> params;

	private Stack<String> body;

	private JarvisClosure() {
	}

	public JarvisClosure(ArrayList<String> p, Stack<String> b) {
		params = p;
		body = b;
	}
	...
	public JarvisAtom interpretNoPut(JarvisInterpreter ji) {
		...
		if (params.size() != ji.getArgCount()) {
			throw new IllegalArgumentException("Bad number of arguments, expected " + params.size()
					+ " got " + (ji.getArgCount()));
		}
		 ...
		 * 
		 */
		ji.pushEval(JarvisCommand.CMD_ENDOFENVIRONMENT);
	...
	public static JarvisClosure read(JarvisInterpreter ji) {
		JarvisClosure closure = new JarvisClosure();
		...
		while (nestingCounter>0) {
			...
			if(s.compareTo(JarvisCommand.CMD_CLOSURE)==0)
			{
				nestingCounter++;
			}
			else if(s.compareTo(JarvisCommand.CMD_ENDOFCLOSURE)==0)
			{
				nestingCounter--;
				if(nestingCounter==0)
				{
	...
	public void interpret(JarvisInterpreter ji) {
		...
		if (!(res instanceof JarvisClosure)) {
			ji.putArg(res);
		}
	}
